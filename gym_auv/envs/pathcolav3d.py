...
class PathColav3d(gym.Env):
    ...
    def __init__(self, env_config, scenario="beginner"):
        ...
        self.n_observations = self.n_obs_states + self.n_obs_errors + self.n_obs_inputs + self.sensor_input_size[
            0] * self.sensor_input_size[1]
        ...
        self.sectors_horizontal = np.linspace(
            -max_horizontal_angle * np.pi / 180,
            max_horizontal_angle * np.pi / 180, self.sensor_suite[0])
        self.sectors_vertical = np.linspace(-max_vertical_angle * np.pi / 180,
                                            max_vertical_angle * np.pi / 180,
                                            self.sensor_suite[1])
    ...
    def generate_environment(self):
        ...
        scenario = self.scenario_switch.get(self.scenario,
                                            lambda: print("Invalid scenario"))
        init_state = scenario()
    ...
    def observe(self, nu_c):
        ...
        if self.total_t_steps % self.update_sensor_step == 0:
            ...
            self.sonar_observations = skimage.measure.block_reduce(
                self.sensor_readings, (2, 2), np.max)
    ...
    def step_reward(self, obs, action):
        ...
        for obstacle in self.nearby_obstacles:
            if np.linalg.norm(obstacle.position - self.vessel.position
                              ) <= obstacle.radius + self.vessel.safety_radius:
                self.collided = True
        ...
        for obstacle in self.nearby_obstacles:
            for i in range(self.sensor_suite[0]):
                ...
                for j in range(self.sensor_suite[1]):
                    ...
                    _, closeness = self.calculate_object_distance(
                        alpha, beta, obstacle)
                    self.sensor_readings[j,
                                         i] = max(closeness,
                                                  self.sensor_readings[j, i])
    ...
    def update_sensor_readings_with_plots(self):
        ...
        """
        print("Time: {}, Nearby Obstacles: {}".format(
            self.total_t_steps, len(self.nearby_obstacles)))
        self.sensor_readings = np.zeros(shape=self.sensor_suite, dtype=float)
        ax = self.plot3D()
        ax2 = self.plot3D()
        for obstacle in self.nearby_obstacles:
            for i in range(self.sensor_suite[0]):
                ...
                for j in range(self.sensor_suite[1]):
                    beta = self.vessel.pitch + self.sectors_vertical[j]
                    s, closeness = self.calculate_object_distance(
                        alpha, beta, obstacle)
                    self.sensor_readings[j,
                                         i] = max(closeness,
                                                  self.sensor_readings[j, i])
                    ...
                    x = self.vessel.position[0] + s * np.cos(alpha) * np.cos(
                        beta)
                    y = self.vessel.position[1] + s * np.sin(alpha) * np.cos(
                        beta)
    ...
    def calculate_object_distance(self, alpha, beta, obstacle):
        ...
        while s < self.sonar_range:
            ...
            z = self.vessel.position[2] - s * np.sin(beta)
            if np.linalg.norm(obstacle.position -
                              [x, y, z]) <= obstacle.radius:
                break
            else:
                s += 1
    ...
    def penalize_obstacle_closeness(self):
        ...
        horizontal_angles = np.linspace(-self.sensor_span[0] / 2,
                                        self.sensor_span[0] / 2,
                                        self.sensor_suite[0])
        vertical_angles = np.linspace(-self.sensor_span[1] / 2,
                                      self.sensor_span[1] / 2,
                                      self.sensor_suite[1])
        for i, horizontal_angle in enumerate(horizontal_angles):
            horizontal_factor = (
                1 - (abs(horizontal_angle) / horizontal_angles[-1]))
            for j, vertical_angle in enumerate(vertical_angles):
                vertical_factor = (1 -
                                   (abs(vertical_angle) / vertical_angles[-1]))
                beta = vertical_factor * horizontal_factor + epsilon
                sensor_suite_correction += beta
                reward_colav += (beta * (1 / (gamma_c * max(
                    1 - self.sensor_readings[j, i], epsilon_closeness)**2)))**2
    ...
    def check_object_overlap(self, new_obstacle):
        ...
        if np.linalg.norm(self.path.get_endpoint() -
                          new_obstacle.position) < new_obstacle.radius + 5:
            return True
        # check if it overlaps already placed objects
        for obstacle in self.obstacles:
            if np.linalg.norm(obstacle.position - new_obstacle.position
                              ) < new_obstacle.radius + obstacle.radius + 5:
                overlaps = True
        return overlaps
    ...
    def scenario_intermediate(self):
        ...
        lengths = np.linspace(self.path.length * 1 / 3,
                              self.path.length * 2 / 3, self.n_int_obstacles)
        for l in lengths:
    ...
    def scenario_proficient(self):
        ...
        lengths = np.random.uniform(self.path.length * 1 / 3,
                                    self.path.length * 2 / 3,
                                    self.n_pro_obstacles)
    ...
    def scenario_advanced(self):
        ...
        while len(
                self.obstacles
        ) < self.n_adv_obstacles:  # Place the rest of the obstacles randomly
            s = np.random.uniform(self.path.length * 1 / 3,
                                  self.path.length * 2 / 3)
            obstacle_radius = np.random.uniform(low=4, high=10)
            obstacle_coords = self.path(s) + np.random.uniform(
                low=-(obstacle_radius + 10),
                high=(obstacle_radius + 10),
                size=(1, 3))
    ...
    def scenario_test(self):
        ...
        self.obstacles.append(
            Obstacle(radius=10, position=self.path(self.path.length / 2)))
        return initial_state
        """
        radius = 6
        for p in points:
