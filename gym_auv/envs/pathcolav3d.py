...
class PathColav3d(gym.Env):
    ...
    def __init__(self, env_config, scenario="beginner"):
        ...
        self.n_observations = (
            ...
            + self.n_obs_errors
            + self.n_obs_inputs
            + self.sensor_input_size[0] * self.sensor_input_size[1]
        )
        ...
        self.sectors_vertical = np.linspace(
    ...
    def generate_environment(self):
        ...
        scenario = self.scenario_switch.get(
            self.scenario, lambda: print("Invalid scenario")
        )
        init_state = scenario()
    ...
    def step_reward(self, obs, action):
        ...
        step_reward = (
            self.lambda_reward * reward_path_following
            + (1 - self.lambda_reward) * reward_collision_avoidance
            + reward_roll
            + reward_control
        )
        ...
        for obstacle in self.nearby_obstacles:
            if (
                np.linalg.norm(obstacle.position - self.vessel.position)
    ...
    def update_nearby_obstacles(self):
        ...
        for obstacle in self.obstacles:
            ...
            distance_vec_BODY = np.transpose(geom.Rzyx(*self.vessel.attitude)).dot(
                distance_vec_NED
            )
            ...
            if (
                distance - self.vessel.safety_radius - obstacle.radius
                <= self.sonar_range
                and abs(heading_angle_BODY) <= self.sensor_span[0] * np.pi / 180
        ...
        for obstacle in self.nearby_obstacles:
            for i in range(self.sensor_suite[0]):
                ...
                for j in range(self.sensor_suite[1]):
                    ...
                    self.sensor_readings[j, i] = max(
                        closeness, self.sensor_readings[j, i]
                    )
    ...
    def penalize_obstacle_closeness(self):
        ...
        horizontal_angles = np.linspace(
            -self.sensor_span[0] / 2, self.sensor_span[0] / 2, self.sensor_suite[0]
        )
        vertical_angles = np.linspace(
            -self.sensor_span[1] / 2, self.sensor_span[1] / 2, self.sensor_suite[1]
        )
            ...
            for j, vertical_angle in enumerate(vertical_angles):
                ...
                reward_colav += (
                    ...
                    * (
                        ...
                        / (
                            gamma_c
                            * max(1 - self.sensor_readings[j, i], epsilon_closeness)
                            ** 2
                        )
                    )
                ) ** 2
    ...
    def check_object_overlap(self, new_obstacle):
        ...
        if (
            np.linalg.norm(self.path.get_endpoint() - new_obstacle.position)
            < new_obstacle.radius + 5
        ):
            return True
        # check if it overlaps already placed objects
        for obstacle in self.obstacles:
            if (
                np.linalg.norm(obstacle.position - new_obstacle.position)
    ...
    def scenario_intermediate(self):
        ...
        lengths = np.linspace(
            self.path.length * 1 / 3, self.path.length * 2 / 3, self.n_int_obstacles
        )
        for l in lengths:
    ...
    def scenario_expert(self):
        ...
        self.current = Current(
            ...
            Vmin=0.5,
            Vmax=1.0,
            Vc_init=np.random.uniform(0.5, 1),
            alpha_init=np.random.uniform(-np.pi, np.pi),
    ...
    def scenario_test_current(self):
        ...
        self.current = Current(
            ...
            beta_init=np.pi / 6,
            t_step=0,
        )  # Constant velocity current (reproducability for report)
        return initial_state
