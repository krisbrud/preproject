...
class AUV3D:
    ...
    def __init__(self, step_size, init_eta, safety_radius=1):
        self.state = np.hstack([init_eta, np.zeros((6,))])  # Start with zero velocity
    ...
    def step(self, action, nu_c):
        ...
        rudder_angle = (
            self.alpha * commanded_rudder + (1 - self.alpha) * prev_rudder_angle
        )
        elevator_angle = (
            self.alpha * commanded_elevator + (1 - self.alpha) * prev_elevator_angle
        )
    ...
    def chi(self):
        """ """
        # Returns the velocity angle in the lateral plane # TODO: In which frame? Relative/absolute?
        # TODO: Verify
