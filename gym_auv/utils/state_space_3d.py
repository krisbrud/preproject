...
# TODO: Rewrite to diag
Ig = np.vstack([np.hstack([I_x, 0, 0]), np.hstack([0, I_y, 0]), np.hstack([0, 0, I_z])])
...
k = (
    (e**4)
    * (beta_0 - alfa_0)
    / ((2 - e**2) * (2 * e**2 - (2 - e**2) * (beta_0 - alfa_0)))
)
...
def M_RB():
    M_RB_CG = np.vstack([np.hstack([m * I3, zero3]), np.hstack([zero3, Ig])])
...
_Ib = Ig - m * geom.S_skew(r_G).dot(geom.S_skew(r_G))
...
def C_RB(nu):
    ...
    C_RB_CO = np.vstack(
        [
            np.hstack(
                [m * geom.S_skew(nu_2), -m * geom.S_skew(nu_2).dot(geom.S_skew(r_G))]
            ),
            np.hstack(
                [
                    m * geom.S_skew(r_G).dot(geom.S_skew(nu_2)),
                    -geom.S_skew(_Ib.dot(nu_2)),
                ]
            ),
        ]
    )
...
def C_A(nu):
    ...
    v = nu[1]
    w = nu[2]
    p = nu[3]  # NOTE: [p, q, r] - not the same as [q, p, r] in paper
    q = nu[4]
    r = nu[5]

    C_11 = np.zeros((3, 3))
    # TODO: Rewrite using S_skew?
    C_12 = np.array(
        [
            [0, -Z_wdot * w, Y_vdot * v],
            [Z_wdot * w, 0, -X_udot * u],
...
def D(nu):
    ...
    return _D + _D_n @ nu.reshape(6, 1) + _L * u
...
def G(eta):
    ...
    # Small optimization: Calculate trig funcs only once
    sin_theta = sin(theta)
    sin_phi = sin(phi)
    cos_theta = cos(theta)
    ...
    G = np.array(
        [
            (W - _B) * sin_theta,
            -(W - _B) * cos_theta * sin_phi,
            -(W - _B) * cos_theta * cos(phi),
            z_G * W * cos_theta * sin_phi,
            z_G * W * sin_theta,
            0,
        ]
    )
