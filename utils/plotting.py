...
def set_default_plot_rc():
    ...
    colors = (cycler(color=[
        '#EE6666', '#3388BB', '#88DD89', '#EECC55', '#88BB44', '#FFBBBB'
    ]) + cycler(linestyle=['-', '-', '-', '--', ':', '-.']))
    plt.rc('axes',
           facecolor='#ffffff',
           edgecolor='black',
...
def plot_attitude(sim_df, output_dir, tracker=None):
    ...
    set_default_plot_rc()
    plt.figure()
    ax = sim_df.plot(x="Time",
                     y=[r"$\phi$", r"$\theta$", r"$\psi$"],
                     kind="line")
...
def plot_control_inputs(sim_dfs, output_dir, tracker=None):
    ...
    plt.legend([r"$\lambda_r=0.9$", r"$\lambda_r=0.5$", r"$\lambda_r=0.1$"],
               loc="upper right",
               fontsize=14)
    plt.ylim([-1.25, 1.25])
...
def plot_3d(env, sim_df, output_dir, tracker=None):
    ...
    ax.plot3D(sim_df[r"$N$"],
              sim_df[r"$E$"],
              sim_df[r"$D$"],
              color="#EECC55",
              label="AUV Path")  #, linestyle="dashed")
...
def plot_collision_reward_function():
    ...
    for i, horizontal_angle in enumerate(horizontal_angles):
        horizontal_factor = (1 -
                             (abs(horizontal_angle) / horizontal_angles[-1]))
